...
import { EXPLORE_TRENDS_MOCK } from '@/lib/constants';
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      <header className="sticky top-0 z-30 glass-header border-b border-border">
        <div className="px-4 py-4 flex items-center space-x-4">
          <div className="flex-1 relative group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground/50 group-focus-within:text-primary transition-colors" />
            <input
              type="text"
              placeholder="Explore interests..."
              className="w-full bg-foreground/5 border-none rounded-full py-2 pl-11 pr-4 focus:bg-transparent focus:ring-1 focus:ring-ring transition-all placeholder-foreground/50 outline-none text-[15px] font-medium h-10"
            />
          </div>
        ...
        <div className="flex text-center overflow-x-auto no-scrollbar scroll-smooth">
          {['For you', 'Trending', 'News', 'Sports', 'Entertainment', 'Science'].map((tab, idx) => (
            <div key={tab} className="flex-1 min-w-[90px] py-4 font-bold hover:bg-white/[0.03] transition-colors cursor-pointer relative text-[14px]">
              <span className={idx === 0 ? "text-foreground" : "text-foreground/50"}>{tab}</span>
              {idx === 0 && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-10 h-1 bg-primary rounded-full shadow-lg shadow-primary/50" />}
            </div>
          ))}
        </div>
      </header>
      ...
      <div className="flex-1 p-3 sm:p-4">
        {/* Main Banner */}
        <div className="relative rounded-2xl h-48 sm:h-64 w-full bg-background/90 overflow-hidden cursor-pointer group mb-6 shadow-2xl">
          <img 
        ...
        <div className="mb-4 flex items-center justify-between px-1">
          ...
          <div className="flex items-center text-primary text-[10px] font-black cursor-pointer hover:underline uppercase tracking-widest">
        ...
        <div className="columns-1 sm:columns-2 gap-4 space-y-4">
          {MASONRY_CONTENT.map((item) => (
            <div 
              key={item.id} 
              className="break-inside-avoid relative bg-foreground/3 border border-border rounded-2xl overflow-hidden group hover:border-white/[0.2] transition-all duration-300"
            >
              {item.image && (
              ...
              <div className="p-3">
                <div className="flex justify-between items-start mb-1">
                  <span className="text-[9px] font-bold text-foreground/50 uppercase tracking-widest">{item.category}</span>
                  <MoreHorizontal className="w-4 h-4 text-neutral-600 cursor-pointer hover:text-foreground transition-colors" />
                </div>
                <h4 className="font-bold text-[14px] transition-colors tracking-tight">{item.title}</h4>
                {item.description && (
                  <p className="mt-1.5 text-xs text-foreground/80 font-medium leading-relaxed line-clamp-2">
                    {item.description}
                  </p>
                )}
                <div className="mt-2.5 flex items-center text-[9px] font-bold text-foreground/50 uppercase tracking-tighter">
        ...
        <div className="mt-10 mb-6">
          ...
          <div className="flex flex-wrap gap-2 px-1">
            {['#Coding', '#ReactJS', '#AI', '#Vibe', '#Fitness', '#Design', '#Startup', '#Travel'].map((tag) => (
              <div 
                key={tag} 
                className="flex items-center space-x-2 bg-white/[0.05] hover:bg-primary/10 hover:text-primary/60 transition-all rounded-full px-4 py-2 cursor-pointer border border-white/[0.08] active:scale-95"
              >
        ...
        <div className="bg-gradient-to-r from-primary/10 to-transparent p-6 rounded-2xl border border-white/[0.08] mb-10 flex flex-col sm:flex-row items-center justify-between text-center sm:text-left gap-4">
          <div>
            <h3 className="text-lg font-black tracking-tight mb-1">Personalize your feed?</h3>
            <p className="text-xs text-foreground/40 font-medium">Follow your favorite topics to stay updated.</p>
          </div>
          <Button size="sm" className="rounded-xl px-6 h-10 font-bold shadow-lg shadow-primary/20 whitespace-nowrap">
            Follow Topics
          </Button>
        </div>
      </div>
