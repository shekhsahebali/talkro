...
import { COMMUNITIES_MOCK } from '../constants';
...
const Communities: React.FC<CommunitiesProps> = ({ onCreate, onSelect }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black">
      <header className="sticky top-0 z-30 glass-header border-b border-white/[0.08]">
        <div className="px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button variant="ghost" size="icon" onClick={onCreate} className="rounded-full bg-sky-500/10 hover:bg-sky-500/20 text-sky-500">
      ...
      <div className="flex-1 flex flex-col">
        <div className="px-4 py-3 flex space-x-2 overflow-x-auto no-scrollbar border-b border-white/[0.08] sticky top-[68px] glass-header z-20">
          <Button variant="secondary" size="sm" className="rounded-full shrink-0 h-9 px-5 font-bold">Discover</Button>
          <Button variant="outline" size="sm" className="rounded-full shrink-0 h-9 px-5 font-bold border-white/[0.1] text-neutral-400">Your Groups</Button>
          <Button variant="outline" size="sm" className="rounded-full shrink-0 h-9 px-5 font-bold border-white/[0.1] text-neutral-400">Events</Button>
        </div>
        ...
        <div className="p-4 space-y-8">
          <section>
            <div className="flex justify-between items-center mb-5">
              ...
              <span className="text-sky-500 text-sm font-bold cursor-pointer hover:underline uppercase tracking-tighter">See all</span>
            </div>
...
>>>>>>> Stashed changes:src/app/(narrow)/communities/page.tsx
                >
                  ...
                  <div className="p-4 relative flex flex-col flex-1">
                    <Avatar className="w-16 h-16 border-4 border-black absolute -top-10 left-4 shadow-xl">
                      <AvatarImage src={group.avatar} />
                      <AvatarFallback>{group.name[0]}</AvatarFallback>
                    </Avatar>
                    
                    <div className="mt-8 flex-1">
                      <h3 className="font-bold text-lg hover:text-sky-400 transition-colors tracking-tight">{group.name}</h3>
                      <p className="text-neutral-500 text-[13px] flex items-center space-x-1 mt-1 font-medium uppercase tracking-tighter">
                        <Globe className="w-3.5 h-3.5" />
                        <span>Public · {group.memberCount} members</span>
                      </p>
                      <p className="text-[14px] mt-4 line-clamp-2 text-neutral-300 leading-relaxed font-medium">
                        {group.description}
                      </p>
                    </div>
                    ...
                    <div className="mt-6 flex space-x-2">
                      <Button 
                        ...
                        className={cn(
                          ...
                          group.isJoined 
                            ? "bg-transparent border border-white/[0.1] text-white hover:bg-white/[0.05] hover:text-rose-500 hover:border-rose-500/50" 
                      ...
                      <Button variant="outline" className="rounded-xl px-3 h-10 border-white/[0.1]" onClick={(e) => e.stopPropagation()}>
          ...
          <section className="bg-gradient-to-br from-white/[0.05] to-transparent rounded-2xl p-6 border border-white/[0.08] flex items-center justify-between">
            ...
            <Button onClick={onCreate} className="rounded-xl px-8 h-11 font-bold shadow-lg shadow-sky-500/10">Start Group</Button>
          </section>
        </div>
      </div>
    </div>
