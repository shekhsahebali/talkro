...
interface SettingsProps {
  ...
  onSensitiveChange: (val: boolean) => void;
}

const SettingsPage: React.FC<SettingsProps> = ({ 
  theme, onThemeChange, 
  fontSize, onFontSizeChange,
  sensitiveContent, onSensitiveChange
}) => {
                ...
                <div className="space-y-4">
                  <div className="bg-white/[0.03] rounded-[32px] p-8 border border-white/[0.08] flex items-center justify-between">
                    ...
                    <div className="flex bg-neutral-900 rounded-2xl p-1 border border-white/5">
                      <button onClick={() => { onThemeChange('light'); showToast('Switched to Light Mode'); }} className={cn("flex items-center px-6 py-2.5 rounded-xl text-xs font-black transition-all", theme === 'light' ? "bg-white text-black shadow-2xl" : "text-neutral-500 hover:text-neutral-300")}>
                        <Sun className="w-4 h-4 mr-2" /> Light
                      </button>
                      <button onClick={() => { onThemeChange('dark'); showToast('Switched to Dark Mode'); }} className={cn("flex items-center px-6 py-2.5 rounded-xl text-xs font-black transition-all", theme === 'dark' ? "bg-sky-500 text-white shadow-2xl" : "text-neutral-500 hover:text-neutral-300")}>
                        <Moon className="w-4 h-4 mr-2" /> Dark
                      </button>
                    </div>
                  </div>

                  <div className="bg-white/[0.03] rounded-[32px] p-8 border border-white/[0.08] space-y-6">
                    <div>
                      <p className="font-black text-lg tracking-tight">Display Font Size</p>
                      <p className="text-xs text-neutral-500 font-medium">Scales text across the entire application interface</p>
                    </div>
                    <div className="flex items-center justify-between space-x-4 bg-neutral-900/50 p-2 rounded-2xl">
                      {(['small', 'medium', 'large'] as const).map(size => (
                        <button 
                          key={size}
                          onClick={() => { onFontSizeChange(size); showToast(`Font size set to ${size}`); }}
                          className={cn(
                            "flex-1 py-3 rounded-xl font-black uppercase text-[10px] tracking-[0.2em] transition-all",
                            fontSize === size ? "bg-sky-500 text-white shadow-lg" : "text-neutral-500 hover:bg-white/5"
                          )}
                        >
                          {size}
                        </button>
               ...
               <section>
                  ...
                  <div className="bg-white/[0.03] rounded-[32px] border border-white/[0.08] overflow-hidden divide-y divide-white/5">
                     <SettingsRow 
                        ...
                        toggle={<Toggle active={sensitiveContent} onToggle={() => { onSensitiveChange(!sensitiveContent); showToast('Content filter updated'); }} />} 
                     />
                     <SettingsRow 
